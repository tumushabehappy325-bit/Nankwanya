...
import { fetchFacilities, fetchDonors, fetchBloodRequests, fetchAlerts, updateRequestStatus, fetchContactDisclosures } from '../services/api';
...
export default function AdminDashboard() {
  ...
  const handleOpenDisclosures = async () => {
    try {
      const res = await fetchContactDisclosures();
      if (res.success) {
        setDisclosures(res.disclosures);
      }
    } catch (e) {
      console.error('Failed to load contact disclosures:', e);
    }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          >
            ...
            SMS Logs ({alerts.length})
          </button>

          {/* Uganda DPPA 2019 Disclosure Audit Log Trigger */}
          <button
            onClick={handleOpenDisclosures}
            className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-medium text-xs transition flex items-center gap-1.5 shadow"
            title="Inspect audit trail of disclosed voluntary donor numbers"
          >
      ...
      {/* Uganda DPPA 2019 Contact Disclosure Audit Modal */}
      {isDisclosureModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
          <div className="bg-slate-950 border border-slate-700 rounded-3xl max-w-2xl w-full p-6 shadow-2xl relative max-h-[85vh] flex flex-col">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-white flex items-center gap-2">
                    Uganda DPPA 2019 — Disclosure Audit Trail
                  ...
                  <p className="text-xs text-slate-400">
                    Verifiable log of every staff request to unmask voluntary donor contact information.
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 font-mono text-xs pr-1">
              {disclosures.length === 0 ? (
                <div className="p-8 text-center text-slate-500 space-y-2">
                  <Lock className="w-8 h-8 text-slate-600 mx-auto opacity-50" />
                  <p className="text-xs text-slate-400 font-sans">No contact disclosures recorded yet.</p>
                  <p className="text-[11px] text-slate-500 font-sans max-w-md mx-auto">
                    Donor phone numbers remain masked across the platform by default to prevent unauthorized contact harvesting and predatory blood brokering.
                  </p>
                </div>
              ) : (
                disclosures.map(d => (
                  <div key={d.id} className="p-3 rounded-xl bg-slate-900 border border-slate-800 space-y-1.5 hover:border-slate-700 transition">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-amber-400 font-bold flex items-center gap-1.5">
                        <ShieldCheck className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                        {d.requestedBy}
                      </span>
                      <span className="text-slate-500">{new Date(d.requestedAt).toLocaleTimeString()} • {new Date(d.requestedAt).toLocaleDateString()}</span>
                    </div>
                    <div className="text-[11px] text-slate-300 flex items-center justify-between">
                      <span>Alert Reference: <strong className="text-slate-100">{d.alertId}</strong></span>
                      <span className="text-[10px] text-slate-500 bg-slate-800 px-2 py-0.5 rounded">Log ID: {d.id}</span>
                    </div>
                  </div>
                ))
              )}
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
              <span className="text-[11px]">Compliant with Section 10 & 20 of Uganda Data Protection and Privacy Act (DPPA 2019)</span>
              <button
                onClick={() => setIsDisclosureModalOpen(false)}
